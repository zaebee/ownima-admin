...
import { ArrowLeft, Mail, Phone, MapPin, Calendar, Star, Loader2, CalendarDays, Activity } from "lucide-react"
    ...
    <div className="flex flex-col gap-6 max-w-6xl mx-auto">
      ...
      <Card className="border-none shadow-sm rounded-xl overflow-hidden bg-muted/20">
        <div className="flex flex-col md:flex-row gap-6 p-6">
          <Avatar className="h-24 w-24 border-4 border-background shadow-md">
            ...
            <AvatarFallback className="text-3xl bg-green-600 text-white font-semibold">{initial}</AvatarFallback>
          </Avatar>
          
          <div className="flex flex-col justify-center flex-1">
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-2xl font-bold">{rider.full_name || "Unnamed"}</h1>
          ...
          <div className="flex gap-4 items-center">
            <div className="flex flex-col items-center justify-center p-3 px-6 bg-background rounded-lg border shadow-sm">
              <div className="flex items-center gap-1.5 text-amber-500 mb-0.5">
                ...
                <span className="text-2xl font-bold text-foreground">{rider.average_rating?.toFixed(1) || "New"}</span>
              </div>
              <p className="text-xs text-muted-foreground font-medium">{rider.rating_count || 0} Reviews</p>
            </div>
          </div>
        </div>
      </Card>
      ...
      <div className="flex items-center gap-2 border-b mt-2 mb-2">
        ...
        >
          ...
          Reservations
          <Badge variant="secondary" className="ml-1 h-5 px-1.5">{rider.total_reservations || 0}</Badge>
        </button>
      </div>
      ...
      {activeTab === "overview" && (
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <div className="md:col-span-2">
            {id && <UserActivityTimeline userId={id} userType="RIDER" />}
          </div>
        </div>
      )}
      ...
      {activeTab === "reservations" && (
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
          {id && <UserReservations userId={id} userType="RIDER" />}
        </div>
      )}
    </div>
  )
