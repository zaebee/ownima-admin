...
export const VehicleFilters: React.FC<VehicleFiltersProps> = ({
  ...
  const hasActiveFilters = () => {
    return filters.status !== undefined || (filters.search && filters.search.trim().length > 0);
  };
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex items-center space-x-2 flex-1 sm:flex-initial">
          <label htmlFor="vehicle-status-filter" className="text-sm font-medium text-gray-700 whitespace-nowrap">
            Status:
          </label>
          <select
          ...
          <div className="flex flex-wrap gap-2">
            {filters.status !== undefined && (
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700">
                Status: {VehicleStatusLabels[filters.status] || 'Unknown'}
              </span>
            )}
