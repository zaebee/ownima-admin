  ...
  return (
    <Card className="hover:shadow-md transition-shadow">
      <CardHeader className="pb-2">
        ...
        <CardDescription>{t('desc', 'engagement')}</CardDescription>
      </CardHeader>
      <CardContent className="mt-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* 30 Days Active Card */}
          <div className="flex flex-col gap-2 p-5 rounded-xl bg-muted/30 border border-border">
            <span className="text-sm font-medium text-muted-foreground">{t('activeDays', 'engagement')}</span>
            ...
            <div className="flex justify-between text-xs mt-3 text-muted-foreground">
              <span className="flex items-center gap-1.5 font-medium"><div className="w-2 h-2 rounded-full bg-blue-500"></div> {t('owners', 'engagement')}: {metrics.users.owners.online_last_30_days || 0}</span>
              <span className="flex items-center gap-1.5 font-medium"><div className="w-2 h-2 rounded-full bg-emerald-500"></div> {t('riders', 'engagement')}: {metrics.users.riders.online_last_30_days || 0}</span>
            </div>
          </div>

          {/* Logins Today Card */}
          <div className="flex flex-col gap-2 p-5 rounded-xl bg-muted/30 border border-border">
            <span className="text-sm font-medium text-muted-foreground">{t('loginsToday', 'engagement')}</span>
            <div className="flex items-end justify-between mt-2">
              <span className="text-4xl font-bold text-foreground">
            ...
            <div className="flex justify-between text-xs mt-3 text-muted-foreground">
              <span className="flex items-center gap-1.5 font-medium"><div className="w-2 h-2 rounded-full bg-blue-500"></div> {t('owners', 'engagement')}: {metrics.users.owners.logins_today || 0}</span>
              <span className="flex items-center gap-1.5 font-medium"><div className="w-2 h-2 rounded-full bg-emerald-500"></div> {t('riders', 'engagement')}: {metrics.users.riders.logins_today || 0}</span>
            </div>
          </div>
        </div>
      </CardContent>
